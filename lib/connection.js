...
function Connection(opts) {
  ...
  if (!mle.enabled() && this.config.connectTimeout) {
...
Connection.createQuery = function createQuery(sql, values, cb, config) {
  ...
  var result =  new Commands.Query(options, cb);
  return result;
};
...
if (mle.enabled()) {
  function parseQueryArguments(connection, sql, values, cb) {
    var query = Connection.createQuery(sql, values, cb);
    query._connection = connection;

    if (!(typeof sql === 'object' && 'typeCast' in sql)) {
      query.typeCast = connection.config.typeCast;
    }

    if (query.sql) {
      query.sql = connection.format(query.sql, query.values);
    }

    if (query._callback) {
      query._callback = wrapCallbackInDomain(connection, query._callback);
    }
    return query;
  }
  ...
  Connection.prototype.query = function query(sql, values, cb) {
    ...
    var queryContext = new Object();
    queryContext.row_count = 0;
    queryContext.col_count = 0;
    queryContext.current_row = new Object();
    queryContext.total_cols = 0;
    queryContext.meta_data = [];
    queryContext.rows = [];
    var myCallbacks = new Object();
    myCallbacks.startRow        = function(ctx) {
      ctx.row_count++;
      ctx.current_row = {};
      ctx.col_count = 0;
    };
    myCallbacks.getString = function(str, ctx) {
      var table_name = ctx.meta_data[ctx.col_count]
      ctx.current_row[table_name] = str;
      ctx.col_count += 1;
    };
    myCallbacks.endRow          = function(ctx) {
      ctx.rows.push(ctx.current_row);
      ctx.current_row = undefined;
      ctx.col_count = 0;
    };
    myCallbacks.startFieldMeta  = function(totalCols,ctx) {
      ctx.total_cols = totalCols;
    };
    myCallbacks.getFieldMeta    = function(tableName, colName, ctx) {
      ctx.meta_data[ctx.col_count] = colName;
      ctx.col_count++;
    };
    myCallbacks.endFieldMeta    = function(ctx) { };
    myCallbacks.querySuccess    = function(affectedRows, ctx) { print('Query OK, Rows affected = ' + affectedRows); };
    myCallbacks.queryFailure    = function(sqlErrorNum, errMsg, ctx) { print('SQL Error code: ' +sqlErrorNum+' Error message = '+errMsg); };
    myCallbacks.serverShutdown  = function(ctx) { print('Server Disappeared !!')};
    mle.env().connector({ synchronous: true }).execute(cmdQuery.sql, queryContext, myCallbacks);
    if (cmdQuery.onResult != undefined) {
      cmdQuery.onResult(undefined, queryContext.rows, []);
    }
  }
